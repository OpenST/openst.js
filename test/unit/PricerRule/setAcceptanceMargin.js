...
describe('PricerRule.setAcceptanceMargin()', () => {
  let pricerRule;
  let web3;
  ...
  it('should pass with correct params', async () => {
    ...
    const acceptanceMargin = '10000000000';
    const mockRawTx = 'mockRawTx';

    const rawTx = sinon.replace(pricerRule, 'setAcceptanceMarginRawTx', sinon.fake.resolves(mockRawTx));
    ...
    const response = await pricerRule.setAcceptanceMargin(payCurrencyCode, acceptanceMargin, txOptions);
    assert.isTrue(response, 'setAcceptanceMargin should return true');
    Spy.assert(rawTx, 1, [[payCurrencyCode, acceptanceMargin]]);
  ...
  it('should throw an error when transaction options is undefined', async () => {
    ...
    await AssertAsync.reject(
      pricerRule.setAcceptanceMargin(payCurrencyCode, acceptanceMargin, txOptions),
