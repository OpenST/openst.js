...
describe('PricerRule.removePriceOracle()', () => {
  let pricerRule;
  let web3;
  ...
  it('should pass with correct params', async () => {
    ...
    const rawTx = sinon.replace(pricerRule, 'removePriceOracleRawTx', sinon.fake.resolves(mockRawTx));
    ...
    const response = await pricerRule.removePriceOracle(payCurrencyCode, txOptions);
    assert.isTrue(response, 'removePriceOracle should return true');
  ...
  it('should throw an error when transaction options is undefined', async () => {
    ...
    await AssertAsync.reject(
      pricerRule.removePriceOracle(payCurrencyCode, txOptions),
