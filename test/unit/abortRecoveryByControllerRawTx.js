...
describe('Recovery.abortRecoveryByControllerRawTx()', () => {
  ...
  it('should return correct raw tx', async () => {
    ...
    const spyRawTx = sinon.replace(recovery.contract.methods, 'abortRecoveryByController', sinon.fake.resolves(mockTx));
    ...
    const response = await recovery.abortRecoveryByControllerRawTx(prevOwner, oldOwner, newOwner);
  ...
  it('should throw an error when previous Owner is invalid', async () => {
    ...
    await AssertAsync.reject(
      recovery.abortRecoveryByControllerRawTx(prevOwner, oldOwner, newOwner),
