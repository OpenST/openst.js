...
describe('UserWalletFactory.constructor()', () => {
  let web3;
  let address;
  ...
  it('should construct with correct parameter', async () => {
    ...
    const spyContract = sinon.replace(Contracts, 'getUserWalletFactory', sinon.fake.returns(fakeInstance));

    const instance = new UserWalletFactory(web3, address);
  ...
  it('should throw an error when getUserWalletFactory returns undefined object', async () => {
    const spyContract = sinon.replace(Contracts, 'getUserWalletFactory', sinon.fake.returns(undefined));

    const errorMessage = `Could not load UserWalletFactory contract for: ${address}`;
    await AssertAsync.reject(new UserWalletFactory(web3, address), errorMessage);
  ...
  it('should throw an error when web3 object is undefined', async () => {
    await AssertAsync.reject(
      new UserWalletFactory(undefined, address),
  ...
  it('should throw an error when recovery contract address is undefined', async () => {
    await AssertAsync.reject(
      new UserWalletFactory(web3, undefined),
