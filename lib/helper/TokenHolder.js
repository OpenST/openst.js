...
class TokenHolder {
  ...
  getDirectTransferExecutableData(transferTo, transfersAmount) {
    ...
    const jsonInterface = oThis.abiBinProvider.getABI(tokenRulesContractName),
      tokenRuleContract = new oThis.auxiliaryWeb3.eth.Contract(jsonInterface, oThis.tokenRules),
      directTransferExecutable = tokenRuleContract.methods.directTransfers(transferTo, transfersAmount).encodeABI();
