...
class GnosisSafe {
   ...
   * @param gnosisSafeAddress Gnosis safe proxy address of a user.
   */
  constructor(auxiliaryWeb3, gnosisSafeAddress) {
    if (!(auxiliaryWeb3 instanceof Web3)) {
      const err = new TypeError("Mandatory Parameter 'auxiliaryWeb3' is missing or invalid.");
      return Promise.reject(err);
    }

    if (!Web3.utils.isAddress(gnosisSafeAddress)) {
      const err = new TypeError(`Mandatory Parameter 'gnosisSafeAddress' is missing or invalid: ${gnosisSafeAddress}.`);
      return Promise.reject(err);
    }
    ...
    this.contract = Contracts.getGnosisSafe(this.auxiliaryWeb3, this.gnosisSafeAddress);

    if (!this.contract) {
      const err = new TypeError(`Could not load gnosisSafe contract for: ${this.gnosisSafeAddress}`);
      return Promise.reject(err);
    }
  }
  ...
  static async deploy(auxiliaryWeb3, txOptions) {
    ...
    const tx = GnosisSafe.deployRawTx();
  ...
  async execTransaction(
    ...
    const txObject = this.execTransactionRawTx(
      to,
      value,
      data,
      operation,
  ...
  async getTransactionHash(to, value, data, operation, safeTxGas, dataGas, gasPrice, gasToken, refundReceiver, nonce) {
    ...
    const nonceFromContract = await this.contract.methods.nonce().call();
  ...
  getSafeTxData(to, value, data, operation, safeTxGas, dataGas, gasPrice, gasToken, refundReceiver, nonce) {
    ...
    const typedDataInput = {
      ...
      primaryType: 'SafeTx',
      domain: {
        verifyingContract: this.gnosisSafeAddress
      },
      message: {
        to: to,
