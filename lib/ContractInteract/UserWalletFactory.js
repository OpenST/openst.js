...
/**
 * Class to interact with UserWalletFactory contract.
 */
class UserWalletFactory {
   ...
   * @param userWalletFactoryAddress Gnosis safe proxy address of a user.
   */
  constructor(auxiliaryWeb3, userWalletFactoryAddress) {
    if (!(auxiliaryWeb3 instanceof Web3)) {
      const err = new TypeError("Mandatory Parameter 'auxiliaryWeb3' is missing or invalid.");
      return Promise.reject(err);
    }

    if (!Web3.utils.isAddress(userWalletFactoryAddress)) {
      const err = new TypeError(
        `Mandatory Parameter 'userWalletFactoryAddress' is missing or invalid: ${userWalletFactoryAddress}.`
      );
      return Promise.reject(err);
    }

    this.auxiliaryWeb3 = auxiliaryWeb3;
    this.userWalletFactoryAddress = userWalletFactoryAddress;

    this.contract = Contracts.getUserWalletFactory(this.auxiliaryWeb3, this.userWalletFactoryAddress);

    if (!this.contract) {
      const err = new TypeError(
        `Could not load userWalletFactoryAddress contract for: ${this.userWalletFactoryAddress}`
      );
      return Promise.reject(err);
    }
  }
  ...
  static async deploy(auxiliaryWeb3, txOptions) {
    ...
    const tx = UserWalletFactory.deployRawTx();
