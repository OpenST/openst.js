...
class ProxyFactory {
   ...
   * @param proxyFactoryAddress Gnosis safe proxy address of a user.
   */
  constructor(auxiliaryWeb3, proxyFactoryAddress) {
    if (!(auxiliaryWeb3 instanceof Web3)) {
      const err = new TypeError("Mandatory Parameter 'auxiliaryWeb3' is missing or invalid.");
      return Promise.reject(err);
    }

    if (!Web3.utils.isAddress(proxyFactoryAddress)) {
      const err = new TypeError(
        `Mandatory Parameter 'proxyFactoryAddress' is missing or invalid: ${proxyFactoryAddress}.`
      );
      return Promise.reject(err);
    }
    ...
    this.contract = Contracts.getProxyFactory(this.auxiliaryWeb3, this.proxyFactoryAddress);

    if (!this.contract) {
      const err = new TypeError(`Could not load proxyFactoryAddress contract for: ${this.proxyFactoryAddress}`);
      return Promise.reject(err);
    }
  }
  ...
  static async deploy(auxiliaryWeb3, txOptions) {
    ...
    const tx = ProxyFactory.deployRawTx();
