...
class OpenSTContracts extends Contracts {
  ...
  static getTokenRules(auxiliaryWeb3, address, options) {
    const oThis = this;
    const jsonInterface = abiBinProviderObject.getABI('TokenRules');
    const contract = new oThis.auxiliaryWeb3.eth.Contract(jsonInterface, address, options);
    return contract;
  }
  ...
  static getPricerRule(auxiliaryWeb3, address, options) {
    const oThis = this;
    const jsonInterface = abiBinProviderObject.getABI('PricerRule');
    const contract = new oThis.auxiliaryWeb3.eth.Contract(jsonInterface, address, options);
    return contract;
  }
  ...
  static getTokenHolder(auxiliaryWeb3, address, options) {
    const oThis = this;
    const jsonInterface = abiBinProviderObject.getABI('TokenHolder');
    const contract = new oThis.auxiliaryWeb3.eth.Contract(jsonInterface, address, options);
    return contract;
  }
  ...
  static getGnosisSafe(auxiliaryWeb3, address, options) {
    const oThis = this;
    const jsonInterface = abiBinProviderObject.getABI('GnosisSafe');
    const contract = new oThis.auxiliaryWeb3.eth.Contract(jsonInterface, address, options);
    return contract;
  }
  ...
  static getDelayedRecovery(auxiliaryWeb3, address, options) {
    ...
    const contractInstance = new oThis.auxiliaryWeb3.eth.Contract(jsonInterface, address, options);
    return contractInstance;
  }
